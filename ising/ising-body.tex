...
\section{Conclusion}
We find that SAD, 1/t-WL, and WL followed by a production run
...
not suffer from
error saturation. We find that for larger Ising systems SAD reduces the update
factor more slowly (and conservatively) than 1/t-WL and WL followed by a
production run. This means that SAD will take
proportionately more moves to converge to the same value as 1/t-WL as system
size is increased (WL methods are given the energy range \emph{a priori.} and
converge independent of system size). SAD requires the user to specify a
temperature range of interest rather than an energy range of interest as the WL
methods do. The 2D
