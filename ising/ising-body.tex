...
A problem that flat-histogram methods face is that the convergence rate is
impacted by energy barriers and bottleneck which make traversing the phase
space difficult. Nadler \emph{et al.} and Trebst \emph{et al.} systematically
examined optimized ensembles due to performance issues flat-histogram methods
faced when confronted with hidden energy barriers~\cite{nadler2007generalized,
nadler2007dynamics, trebst2004optimizing}. The goal of these methods is to make
the statistical errors uniform such that low energy errors do not dominate over
high energy errors.

% replica exchange paragraph

Parallel tempering (replica exchange) is a powerful method for overcoming
energy barriers~\cite{geyer1995annealing, hukushima1996exchange,
hansmann1997parallel}. The method works by breaking the simulation up into many
`replicas' and simulating in parallel. At certain times, neighboring replicas
are allowed to exchange which enables the phase space to be more evenly
explored.

Flat-histogram methods, such as Replica exchange Wang-Landau
(REWL)~\cite{vogel2013rewl,perera2016magnetic}, have been proposed that combine
histogram and replica exchange methods. This demonstrates that flat-histogram
methods can be extended to non-flat-histogram methods.
