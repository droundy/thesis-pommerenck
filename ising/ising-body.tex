...
\davidsays{let's keep replica exchange and optimized ensemble in separate paragraphs}
...
optimized ensemble to systematically set the temperature range(s) and
implemented an optimized replica-exchange~\cite{trebst2006optimized}
\davidsays{looks like this is not a flat-histogram approach, scrap this reference}.
...
Replica exchange Wang-Landau (REWL) was introduced \davidsays{Need to discuss this, and each of these algorithms
are sufficiently similar that this approach should apply to them.}\cite{vogel2013rewl}.
