...
set of possible materials \cite{Moosavi2020understanding}. To further explore material space and address
...
thermodynamic model for a two-phase adsorbent. Let $\Delta
F=F_{\text{open}}-F_{\text{closed}}$ be the Helmholtz free energy difference per
...
in the pressure of the gas, the material will collapse and expel all residual
...
so that eqn.~\ref{eq:xi_vs_xi0} holds for the open phase of the material.  This
approximate model of the two-phase material has no upper bound on the
deliverable capacity, since $\V$ and $\Delta F$ can together be increased
without bound.  We can, however, place a bound for a fixed $\Delta F$, since if
the gas is too strongly attracted to the open phase, the material will remain
open at $\pempty$ and our rigid upper bound applies.
...
In
terms of $\rho_g(\mu, T)$, then, the grand potential $\Omega$ in our idealized
substrate is:
\begin{equation}
    \Omega(T, \mu) = - \int_{-\infty}^\mu \rho_g(\mu-\V)d\mu.
\end{equation}
However, the \emph{osmotic potential} governs which phase the material adopts.
The material changes phases when osmotic potentials of the two phases are equal.
% At the pressure at which they switch when the osmotic potentials in the two phases
% are equal.
...
%
This means the material should change phases at pressure $\pempty$, which gives
\begin{align}
    % F_{closed} + 0 &= F_{open} + \Omega(T, \mu) \\
    \frac{\Delta F}{V_{\text{open}}} &= \int_{-\infty}^{\muempty} \rho_g(\mu-\V_{opt})d\mu.
\end{align}
...
%under the assumption that the mechanical
%work associated with the transition is neglible when compared with the
%adsorption energies involved.
I.e., (1, to be open at $\pfull$) the gas-host attractions at $\pfull$ must
overcome the free energy penalty to open the structure and (2, to be closed at
$\pempty$) the free energy penalty to open the structure must be larger than the
(weak) gas-host attractions at $\pempty$. This approximate model of the
two-phase material has no upper bound on the deliverable capacity, since $\V$
and $\Delta F$ can together be increased without bound. We can, however, place a
bound for a fixed $\Delta F$, since if the gas is too strongly attracted to the
open phase, the material will remain open at $\pempty$ and our rigid upper bound
applies.
