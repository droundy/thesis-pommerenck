...
\end{figure}
While our deliverable capacity limit does not apply to materials that change conformation in
response to gas adsorption, here we adapt our theoretical framework to pertain to bistable adsorbents modeled after Co(bdp) \cite{mason2015methane}.
Actuated by the presence of gas, Co(bdp) can switch between two distinct structural phases:
an ``open'' phase where gas molecules can fit in the pore space and a 
collapsed, ``closed'' phase in which no gas can fit inside.

We follow Coudert et al. \cite{coudert2008thermodynamics} in developing a thermodynamic model for a two-phase adsorbent. 
...
In addition, assume the open phase hosts a uniform, attractive potential energy field $\V<0$ for gas molecules to adsorb, so that eqn.~\ref{eq:xi_vs_xi0} holds for the open phase of the material.
