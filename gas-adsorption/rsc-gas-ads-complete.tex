...
$\gst$ is the ... \davidsays{wut? energy it takes to do this or?}
% David: what is GST
...
\begin{figure}
    ...
    \caption{Deliverable capacity of methane in a flexible substrate at 298\ K as a function of the free energy difference 
...
\end{figure}
Although our limit does not directly apply to materials that change conformation in
response to gas adsorption, we can make an estimate of the benefits to be gained
materials such as \davidsays{FIXME AND CITE} which undergo a phase change in the
presence of gas.  We can model such a material as a two-phase system with one ``open''
phase and a ``closed'' phase in which all gas is expelled.  We assume there is a
free energy difference $\Delta G$ between the two phases in the absence of gas, with
the closed phase more stable.  In addition, we assume a $\V$ as above \davidsays{REFERENCE EQUATION?}
for the adsorption of gas into the open phase.  In order for the system to benefit
from the two phases, the open phase must be preferred at $\pfull$, with the closed phase
preferred at $\pempty$.  This requires that
\begin{align}
    V_{\text{open}}|\rho_{\text{open}}(\pfull)\V| &> |\Delta G| \\
    V_{\text{open}}|\rho_{\text{open}}(\pempty)\V| &< |\Delta G|
\end{align}
where $\rho_{\text{open}}(p)$ is the density of gas in the open phase of the material at
the given pressure.  This model has no upper bound on the deliverable capacity, since
$\V$ and $\Delta G$ can together be increased without bound.  We can, however, place a
bound for a fixed $\Delta G$, since if the gas is too strongly attracted to the open phase
the material will remain open at $\pempty$.  This upper bound is found by identifying the
attractive interaction $\V$ that solves the equation
\begin{align}
    \rho(\muempty{} + \V)\V = \Delta G/V_{\text{open}}
\end{align}
for $\V$, which gives the largest attraction consistent with the material closing
at the empty pressure.
Figure~\ref{fig:methane-flexible} illustrates
how this upper bound depends on $\Delta G$ for methane.  For comparison, we include the free energy
difference for Co(bdp)~\davidsays{CITE} and its measured deliverable capacity.  While this
upper bound is not rigorous, it suggests that a two-phase flexible material for methane storage
...
reach the DOE ULTIMATE deliverable capacity would require far greater stability for the closed
