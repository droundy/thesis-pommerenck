...
\section{Flexible two-state model}

\begin{align}
    V_{\text{open}}|n(p_{hi})\V| &> |\Delta F| \\
    V_{\text{open}}|n(p_{lo})\V| &< |\Delta F|
\end{align}
This means that for a given $\Delta F$ we can put an upper bound on the
deliverable capacity.

\davidsays{This means we should pick the $\V$ to be big enough to make us just
on the boundary of not collapsing.  This means that:}
\begin{align}
    n(\mu_{lo} + \V)\V = \Delta F
\end{align}
This is a nonlinear equation that we want to solve.
